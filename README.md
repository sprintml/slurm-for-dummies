...
Do it simply by:
```
scp /etc/munge/munge.key root@sprint2:/etc/munge/munge.key
```
...
Solve the problem with permission rights:

# create files for slurm on each node:
```
cd /
mkdir slurm
mkdir -p slurm/spool/slurmd
mkdir -p slurm/spool/slurmctld
mkdir -p slurm/run
mkdir -p slurm/log
chown slurm:slurm -R slurm
```
...
#
ClusterName=sprint
SlurmctldHost=sprint1
SlurmctldHost=sprint2
#
#DisableRootJobs=NO
...
#
# COMPUTE NODES
NodeName=sprint1 NodeHostName=sprint1 NodeAddr=10.17.161.12 CPUs=112 RealMemory=515404 Sockets=2 CoresPerSocket=28 ThreadsPerCore=2 State=UNKNOWN
PartitionName=sprint Nodes=ALL Default=YES MaxTime=INFINITE State=UP Gres=gpu:4
NodeName=sprint2 NodeHostName=sprint2 NodeAddr=10.17.161.13 CPUs=112 RealMemory=515404 Sockets=2 CoresPerSocket=28 ThreadsPerCore=2 State=UNKNOWN
PartitionName=sprint Nodes=ALL Default=YES MaxTime=INFINITE State=UP Gres=gpu:4
```
